...
// simbolo mais a esquerda das regras de produção será o simbolo inicial.
...
func (generator *Generator) BuildParser() *Parser {
	...
	i0 := newItemSet()
	i0.add(item{0, 0}) // primeira produção
	...
	for itemSetId, itemSet := range generator.itemSets {
		for symId, gotoSetId := range itemSet.gotos {
			table[symId][itemSetId] = gotoSetId + 1 // shift para terminais e goto para não terminais
		}
